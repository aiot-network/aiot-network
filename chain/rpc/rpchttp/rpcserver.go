...
func NewRPCServer(cfg *config.RpcConfig) (*RpcServer, error) {
	rpc := RpcServer{
...
func (s *RpcServer) Start(rpcListeners []string) error {
	//TODO control by config
	if err := s.startHTTP(rpcListeners); err != nil {
		return err
	}
	s.run = 1
	return nil
...
func (s *RpcServer) limitConnections(w http.ResponseWriter, remoteAddr string) bool {
	if int(atomic.LoadInt32(&s.numClients)+1) > RPCMaxClients {
		log.Info("RPC clients exceeded", "max", RPCMaxClients,
			"client", remoteAddr)
