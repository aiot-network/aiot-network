...
import (
	...
	"github.com/aiot-network/aiotchain/tools/network"
	"github.com/aiot-network/aiotchain/tools/roughtime"
...
// both slices.
func parseListeners(cfg *config.RpcConfig, addrs []string) ([]net.Listener, error) {
	...
	if cfg.RpcTLS {
		...
		// exist.
		if !util.FileExists(cfg.RpcCertKey) && !util.FileExists(cfg.RpcCert) {
			err := genCertPair(cfg.RpcCert, cfg.RpcCertKey)
			if err != nil {
				return nil, err
			}
		}
		keypair, err := tls.LoadX509KeyPair(cfg.RpcCert, cfg.RpcCertKey)
		if err != nil {
			return nil, err
		}
